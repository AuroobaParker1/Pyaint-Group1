...
class Button:
    ...
    def draw(self, win):
        ...
        if self.image_url != "/":
            if (self.name != "Brush" and self.name != "FillBucket"):
                pygame.draw.rect(win, BLACK, (self.x-2, self.y-2, self.width+4, self.height+4))
                pygame.draw.rect(win, BLACK, (self.x-2, self.y-2, self.width+4, self.height+4), 2)
